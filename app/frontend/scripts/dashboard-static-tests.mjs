...
const checks = [
  ...
  ['Local PDF upload creates a browser preview URL', dashboard, "previewUrl: kind === 'image' || kind === 'pdf' ? URL.createObjectURL(file) : null"],
  ['Unsaved local PDFs render before upload', dashboard, 'selected.previewUrl || selected.serverPreviewUrl'],
  ['PDF thumbnails do not treat PDF blob previews as images', dashboard, "file.kind === 'image' ? file.thumbnailUrl || file.previewUrl : file.thumbnailUrl"],
  ['Dashboard PDF preview keeps native viewer for page navigation', dashboard, 'nativePdfPreviewUrl'],
  ['Dashboard PDF preview does not force first rendered page', dashboard, 'documentPreviewPageUrl(selected.documentId, 1)', true],
  ...
  ['Dashboard native PDF preview ignores app click zoom', dashboard, 'if (usesNativePdfViewer) return'],
  ['Dashboard native PDF preview does not apply shell zoom', dashboard, 'const effectiveZoom = usesNativePdfViewer ? 100 : zoom'],
  ['Dashboard native PDF preview disables shell rotation', dashboard, "transform: usesNativePdfViewer ? 'none'"],
  ['Dashboard native PDF preview explains native toolbar', dashboard, 'Use the PDF toolbar for pages and zoom.'],
  ['Upload PDF zoomed mode is unclamped', styles, 'Upload PDF preview zoom repair'],
  ['Dashboard PDF preview iframe has explicit class', dashboard, 'upload-native-pdf-preview'],

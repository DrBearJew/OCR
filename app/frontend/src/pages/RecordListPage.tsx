    ...
    <main className="records-console">
      ...
      <section className="workflow-card record-workflow-panel">
        <form className="upload-band record-upload-band" onSubmit={upload}>
          ...
          <input placeholder="Record title" value={label} onChange={(event) => setLabel(event.target.value)} />
          <input type="file" multiple onChange={(event) => setFiles(event.target.files)} />
          <button className="primary" disabled={busy || !files?.length}><Upload size={18} /> Upload</button>
        </form>
      </section>
      ...
      <section className="workflow-card record-browser-panel">
        <div className="document-toolbar record-toolbar">
          <label className="toolbar-search">
            <Search size={17} />
            <input placeholder="Search records, files, generated titles..." value={filters.query || ''} onChange={(event) => setFilters({ ...filters, query: event.target.value })} />
          </label>
          <input placeholder="Collection" value={filters.collection || ''} onChange={(event) => setFilters({ ...filters, collection: event.target.value })} />
